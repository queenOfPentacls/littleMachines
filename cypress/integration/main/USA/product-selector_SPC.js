...
describe('SELECT YOUR HARDWARE - SUPERPOD CHAMPAGNE', () => {
  it('TEST ORDER: 3x SUPERPOD CHAMPAGNE 2 YEAR MEMBERSHIP', { browser: 'electron' }, () => {
	//Store Page
	cy.visit('/')	
	cy.wait(1000)
	...
	cy.get(FOOTER).contains('SuperPod (Champagne)')
	...
	cy.get(FOOTER).contains('SuperPod (Champagne) x3')
	...
	cy.get(CART_ITEM).contains('Champagne')
	...
	cy.get('.cart_item').contains('2 Year - HomePass Membership')
	cy.get('.cart_item').contains('3 SuperPod (Champagne) ')
	...
	cy.get('.woocommerce-order-details').contains('SuperPod 3 pack (Champagne)')
  })

it('TEST ORDER: 3x SUPERPOD CHAMPAGNE 1 YEAR MEMBERSHIP', { browser: 'electron' }, () => {
	cy.clearCookies()
	//Store Page
	cy.visit('/')	
	cy.wait(1000)
...
it('TEST ORDER: 3x SUPERPOD CHAMPAGNE 3 YEAR MEMBERSHIP', { browser: 'electron' }, () => {
	//Store Page
	cy.visit('/')	
	cy.wait(1000)
	cy.get('.store-nav-wrapper>a').eq(0).click()
